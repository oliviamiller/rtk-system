// Package gpsnmea implements an NMEA serial gps for a gpsrtki2cnonetwork.
package nmea
...
func (cfg *Config) Validate(path string) ([]string, error) {
	if cfg.I2CBus == 0 {
		return []string{}, utils.NewConfigValidationFieldRequiredError(path, "i2c_bus")
	}
	if cfg.I2cAddr == 0 {
		return []string{}, utils.NewConfigValidationFieldRequiredError(path, "i2c_addr")
	}
	...
	resource.RegisterComponent(
		...
		resource.Registration[movementsensor.MovementSensor, *Config]{
			Constructor: func(
				...
				return NewNMEAGPS(ctx, deps, conf.ResourceName(), newConf, logger)
			},
		})
}
